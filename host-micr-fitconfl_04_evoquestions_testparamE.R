##
#goal of this script is to create figures from 02 series simulations
##
...
reps = 5 #MUST SET REPLICATES, and must be equal to the number used in 03 and 02 series

pars <- simsens[,1:17]
resps <- simsens[,18:26]
#not all parameters were modified, mpars = columns for modified parameters only
...
#there may be some NA values -- there is 0 additive genetic variance, so it can't calculate a proportion or a fitness correlation
...
indices <- c(1,3,5,6) #parts of responses that we are including.
