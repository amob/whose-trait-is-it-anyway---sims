...
##how to enact genome space in a way that makes sense.
...
HOLOgeno <- cbind( rep(0,times=nrow(expset$expdat)), 
	   ...
	   t(rbind(expset$causalgenos$genoP,
			   expset$neutralgenos$genoP)+1) [expset$expdat$IDPG,] , # concatenating plant genotype matrix, flipping, adding 1 so bt 1 and 2, and then grabbing rows as they are used in the experiment.
 	   t(rbind(expset$causalgenos$genoM[,rep(1:ncol(expset$causalgenos$genoM) ,each=2)],
 	   		   expset$neutralgenos$genoM[,rep(1:ncol(expset$causalgenos$genoM) ,each=2)])+1) [expset$expdat$IDMG,]  # duplicating columns for pretend diploid microbe genotype matrix, concatenating, flipping, adding 1 so bt 1 and 2, and then grabbing rows as they are used in the experiment.
 	   )
...
#linkage group must be alphanumeric not numeric if > 22 chromosomes. so paste "scaffold" first "--allow-extra-chr" flag to make plink accept them.
		 c(paste("cP",1:nrow(expset$causalgenos$locusdatM),sep=""),paste("nP",1:nrow(expset$neutralgenos$locusdatM),sep="") ),#      rs# or snp identifier
