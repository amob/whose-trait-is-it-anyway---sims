##
#goal of this script is to create a figure from 05 series simulations
##
...
reps <- 5 ## MUST SET NUMBER OF REPS. -- this is as run in 05/06 series of rscripts
...
for(i in rofint){
	for(j in unique(parbyreps$zoP)){
		if(i ==1){
			...
			ranges <- c(-0.3,3.3)
			} else if(i == 5){ #high variance in this one, its a "real" phenomena, but makes visualizing difficult
			ranges <- c(0,0.05) # plot log instead to avoid issue
			} else {ranges <- bufferX(range(respsbyreps[,i]),0.1)} #; keeping on same scale means only 1 y axis
