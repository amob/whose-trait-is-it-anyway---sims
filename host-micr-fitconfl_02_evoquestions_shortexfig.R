...
# dev.off()
# 
# ###THESE ARE THE ONES THAT GET RUN
...
# 	mtext("No link to microbe fitness",side=3,line=0.5)
...
# windowplot(1,gens+1, 3,test.4a,ylim=c(-3,6),"",xlabs="")#titles[3])
# 	abline(h=3,col=rgb(0,0,0),lty=2); # abline(h=2,col=rgb(0,0.5,0),lty=2)#col=rgb(1,0,0),lty=2)
# 	abline(h=0)
# 	mtext("Equal links to fitness",side=3,line=0.5)
# 	mtext("Same optima",side=3,line=2)
# windowplot(1,gens+1, 3,test.4aff,ylim=c(-3,6),"")#"titles[4]")
# 	abline(h=3,col=rgb(0,0,0),lty=2); #abline(h=2,col=rgb(0,0.5,0),lty=2)#col=rgb(1,0,0),lty=2)
# 	abline(h=0)
...
# 	abline(h=3,col=rgb(0,0.5,0),lty=2); abline(h=2,col=rgb(0.5,0,0.5),lty=2); abline(h=2.5,col=rgb(0,0,0,alpha=.5),lty=2) 
# 	abline(h=0)
# 	legend(gens*-0.05,y=0,c("Host optima","Microbe optima","Midpoint","Both optima"), lty = 2, col = c(rgb(0,0.5,0),rgb(0.5,0,0.5),rgb(0,0,0,alpha=0.5),rgb(0,0,0)), bty="n")
# dev.off()
# 
# 
...
# # conend.e <- c(trait.eff$traitexp[which(trait.eff$rfitmicr == max(trait.eff$rfitmicr))],trait.eff$traitexp[which(trait.eff$rfitplnt== max(trait.eff$rfitplnt))])
# # conend.f <- c(trait.fff$traitexp[which(trait.fff$rfitmicr == max(trait.fff$rfitmicr))],trait.fff$traitexp[which(trait.fff$rfitplnt== max(trait.fff$rfitplnt))])
# # 
...
# plot( trait.f$rfitplnt~trait.f$traitexp,col=rgb(0,0.5,0), pch=1,xlim=c(1,4),ylim=c(0,0.015),cex=0.5)
# 	points( trait.f$rfitmicr~trait.f$traitexp,col=rgb(0.5,0,0.5),pch=1 ,cex=0.5)
# 	points( trait.fff$rfitplnt~trait.fff$traitexp,col=rgb(0,0.5,0),pch=8 ,cex=0.5)
# 	mtext("Relative fitness",side=2,line=2)
# 	points( trait.fff$rfitmicr~trait.fff$traitexp,col=rgb(0.5,0,0.5),pch=8 ,cex=0.5)
