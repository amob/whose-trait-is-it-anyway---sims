...
# # #Temporary commented out, because they take a very long time to run on a personal labptop and this is not recommended.
gens <- 300
# #4 scenarios
...
windowplot(1,gens+1, 3,test.4b,ylim=c(-3,6),"",xlabs="",ylabs="")#titles[3])
	...
	mtext("breeding values",side=2,line=2,adj=-1.65)
...
windowplot(1,gens+1,3,test.4fff,ylim=c(-3,6),"",xlabs="",ylabs="")#titles[10])
	abline(h=3,col=rgb(0,0.5,0),lty=2); abline(h=2,col=rgb(0.5,0,0.5),lty=2); abline(h=2.5,col=rgb(0,0,0,alpha=.5),lty=2) 
	abline(h=0)
	legend(gens*-0.05,y=0,c("Host optimum","Microbe optimum","Midpoint","Both optima"), lty = 2, col = c(rgb(0,0.5,0),rgb(0.5,0,0.5),rgb(0,0,0,alpha=0.5),rgb(0,0,0)), bty="n")
dev.off()
...
par(oma=c(2,2,3,0))
	plot(VmVp4b$Vp ~ c(1:length(VmVp4b$Vp)),pch=NA,ylim=c(0,0.1),ylab="",xlab="")
		...
		mtext("genetic variance",side=2,line=2,adj=-1.7)
...
plot( unlist(soj4bM$effs)[unlist(soj4bM$effs)!=0 & is.na(unlist(soj4bM$soj))] ~
	...
	abline(h=0)
 	mtext("No fitness feedback",side=2, line=3.5,cex=1.25)
	mtext("No direct link to microbe fitness",side=3,line=0.5)
	mtext("effect size",side=2,line=2,adj=-0.4)
