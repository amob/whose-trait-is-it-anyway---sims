...
examplefinal <- read.csv(file=paste(Sys.getenv("SCRATCH"),'/feedback_stats/sensitivity_stats',1,'rep',1,'.csv',sep=""),header=T)
...
for(i in 1:nrow(parm)){
	for(r in 1:10){
		simout <- read.csv(file=paste(Sys.getenv("SCRATCH"),'/feedback_stats/sensitivity_stats',i,'rep',r,'.csv',sep=""),header=T)
...
write.csv(parmWfs,file=paste(Sys.getenv("SCRATCH"),"/sens_reps_feedbackparameters.csv",sep=""),row.names=F)
