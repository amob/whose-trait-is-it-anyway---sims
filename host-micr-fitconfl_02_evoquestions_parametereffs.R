...
basevals <- c(2000,2000, 20,40, 3,3, 3,2,      0.75,0.75, 1000,       25, 0.0001,      0.2,    0.6,0.6)
#sim.cotrait(NP,NM,nlP,nlM,nlnP,nlnM,zoP,zoM,wP,wM,
													#timesteps,Lambda,mutprb,prbHorz, pfP, pfM,startmats = "n",zoptvects = "n")
##since one simulation generates a datafile of about 5MB on disk, then 200 will be ~1000 MB, or ~1 GB.


#setting out parameter modifications to simulate
...
parm[31:40,3] <- nloc.v #P when manipulating P and M tog
parm[31:40,4] <- nloc.v*2 #M when manipulating P and M tog
parm[41:50,9] <- w.v #P
parm[51:60,10] <- w.v #M
