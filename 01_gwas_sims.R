...
write.table(HOLOgeno,"/scratch/f/freder19/annamob/HOLOevosims.ped",quote=F,sep="\t",row.names=F,col.names=F)
write.table(PLANTgeno,"/scratch/f/freder19/annamob/PLANTevosims.ped",quote=F,sep="\t",row.names=F,col.names=F)
write.table(MICRgeno,"/scratch/f/freder19/annamob/MICRevosims.ped",quote=F,sep="\t",row.names=F,col.names=F)
...
#linkage group must be alphanumeric not numeric if > 22 chromosomes. so paste "scaffold" first "--allow-extra-chr" flag to make plink accept them.
		 	...
		 	rep(0,times=nrow(expset$causalgenos$locusdatM)+ nrow(expset$neutralgenos$locusdatM)), #      Genetic distance (morgans) # For basic association testing, the genetic distance column can be set at 0. 
		 	c(expset$causalgenos$locusdatM$location,expset$neutralgenos$locusdatM$location) ) #      Base-pair position (bp units)
write.table(HOLOmap,"/scratch/f/freder19/annamob/HOLOevosims.map",quote=F,sep="\t",row.names=F,col.names=F)
write.table(PLANTmap,"/scratch/f/freder19/annamob/PLANTevosims.map",quote=F,sep="\t",row.names=F,col.names=F)
write.table(MICRmap,"/scratch/f/freder19/annamob/MICRevosims.map",quote=F,sep="\t",row.names=F,col.names=F)
